...
export default function App() {
        ...
        if (Array.isArray(data.candles) && data.candles.length > 0) {
          if (simulatorRef.current && tradingEngineRef.current) {
            ...
            if (isAutoTrading && botState !== "HALTED_DEAD") {
              ...
              // Open paper positions continue to be managed while new entries are paused.
              tradingEngineRef.current.onTick(lastCandle, allCandles, false);
            }
  ...
  const handleReviveBot = (recapital?: number) => {
    ...
    const resumed = tradingEngineRef.current.reviveBot(recapital);
    if (resumed) {
      setIsEmergencyModalOpen(false);
      // Re-arming a paper session never silently enables automated entries.
